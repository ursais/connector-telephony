...
class PCS_VOIP(http.Controller):
    ...
    @http.route('/palitto/incomingCall', type='http', auth='public', website=True, sitemap=False)
    def pcs_incoming_calls(self, *args, **kw):
        user = request.env['res.users'].search([('phone', '=', kw.get('CallerID'))], limit=1)
        a = self.create_cdr_record(**kw)
        return request.env['phone.common'].incall_notify_by_login(kw.get('CallerID'), [user.login])

    @http.route('/palitto/Call', type='http', auth='public', website=True, sitemap=False)
    def pcs_outgoing_calls(self, *args, **kw):
        user = request.env['res.users'].search([('phone', '=', kw.get('CallerID'))], limit=1)
        self.create_cdr_record(**kw)
        return request.env['phone.common'].incall_notify_by_login(kw.get('CallerID'), [user.login])

    @http.route('/palitto/missedCall', type='http', auth='public', website=True, sitemap=False)
    ...
    @http.route('/palitto/Completed', type='http', auth='public', website=True, sitemap=False)
    ...
    @http.route('/palitto/heldCall', type='http', auth='public', website=True, sitemap=False)
    ...
    @http.route('/palitto/unheldCall', type='http', auth='public', website=True, sitemap=False)
